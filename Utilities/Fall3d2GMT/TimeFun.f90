...
CONTAINS
  ...
  subroutine get_date(iyr,imo,idy,ihr,imi,date)
    !***************************************************************
    !*
    !*   Gets date in the form 'ddmmmyyyy-hhmm-UTC'
    !*
    !***************************************************************
    implicit none
    integer(ip)       :: iyr,imo,idy,ihr,imi
    character(len=18) :: date
    !
    write(date(1:2),'(i2.2)') idy
    if(imo.eq.1) then
       write(date(3:5),'(a)') 'jan'
    else if(imo.eq.2) then
       write(date(3:5),'(a)') 'feb'
    else if(imo.eq.3) then
       write(date(3:5),'(a)') 'mar'
    else if(imo.eq.4) then
       write(date(3:5),'(a)') 'apr'
    else if(imo.eq.5) then
       write(date(3:5),'(a)') 'may'
    else if(imo.eq.6) then
       write(date(3:5),'(a)') 'jun'
    else if(imo.eq.7) then
       write(date(3:5),'(a)') 'jul'
    else if(imo.eq.8) then
       write(date(3:5),'(a)') 'aug'
    else if(imo.eq.9) then
       write(date(3:5),'(a)') 'sep'
    else if(imo.eq.10) then
       write(date(3:5),'(a)') 'oct'
    else if(imo.eq.11) then
       write(date(3:5),'(a)') 'nov'
    else
       write(date(3:5),'(a)') 'dec'
    end if
    write(date(6:9),'(i4.4)') iyr
    write(date(10:10),'(a1)') '-'

    write(date(11:12),'(i2.2)') ihr
    write(date(13:14),'(i2.2)') imi
    write(date(15:18),'(a4)') '-UTC'
    !
    return
  ...
  subroutine grday(iyr,ijul,imo,iday)
    ...
    if(ijul.lt.1.or.ijul.gt.kday(12,ileap)) then
       call runend('Wrong computation of the gregorian day')
    end if
    !
    do i=1,12
