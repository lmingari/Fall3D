...
MODULE Master
  ...
  !
  logical           :: moisture_correction, vegetation_correction, recalculate_ust
  character(len=15) :: emission_scheme
  integer(ip)       :: nz_emission
  real   (rp)       :: diam_max,tust_cte,w_threshold,emission_factor,z_emission,mindep,roughness_ust
  real(rp),parameter:: mindep0=1d-3                              !Maximum deposit threshold (kg/m3)
...
! 15 Bedrock
! 16 Other (land-ice)
  real(rp),dimension(19),parameter :: maxsmc = (/ 0.395, 0.41 , 0.435, 0.485, 0.485, &
                                                  0.451, 0.42 , 0.477, 0.476, 0.426, &
                                                  0.492, 0.482, 0.451, 1.00 , 0.2  , &   ! Porosity
                                                  0.435, 0.468, 0.2  , 0.339         /)  ! From RUC SOILPARM.TBL table
  !
  !***  Arrays
  !
  ...
  real(rp), allocatable :: soil(:,:)        ! (nx,ny)         Soil category
  real(rp), allocatable :: vegfra(:,:)      ! (nx,ny)         Vegetation Fraction
  real(rp), allocatable :: rmol(:,:)        ! (nx,ny)         Inv. Monin-Ob. L
  ...
  real(rp), allocatable :: emis_mas(:,:,:)  ! (nx,ny,nc    )  Total Emitted mass
