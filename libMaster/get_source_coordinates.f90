subroutine get_source_coordinates(fname,timesec,x,y,z,endsec,istat,message)
  !**************************************************************************
  !*
  ...
  do while(go_on)
     ...
     if((timesec.ge.itime1).and.(timesec.lt.itime2)) then
        ...
        do is = 1,ns
           read(90,*,ERR=101) x(is),y(is),z(is),ivoid,ivoid,ivoid,(rvoid,ic=1,nc)
        end do
     else
        read(90,*,ERR=101) rvoid            ! MFR
        do is = 1,ns
  ...
  !
100 istat = -1
  mymessage = 'get_source_coordinates: error opening the source file '// &
       TRIM(fname)
...
101 istat = -1
  ...
  return
  !
end subroutine get_source_coordinates
