subroutine get_dbs_property_int(fname,attr,value,istat,message)
  !**************************************************************************
  !*
  !*    Gets an integer global attribute from the database netCDF file
  !*
  !*    INPUT:
  ...
  !
101 istat = -1
  mymessage = 'get_dbs_property_int : Error in nf90_open for file '// &
       TRIM(fname)
  message(1:MIN(ilen,s_mess)) = mymessage(1:MIN(ilen,s_mess))
  return
  !
102 istat = -1
  mymessage = 'get_dbs_property_int : Error in f90_get_att for attribute '// &
       TRIM(attr)
  message(1:MIN(ilen,s_mess)) = mymessage(1:MIN(ilen,s_mess))
  return
  !
103 istat = -1
  mymessage = 'get_dbs_property_int : Error in nf90_close '
  message(1:MIN(ilen,s_mess)) = mymessage(1:MIN(ilen,s_mess))
  return
  !
end subroutine get_dbs_property_int
