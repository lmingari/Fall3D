subroutine get_source_nclass(fname,timesec,nc,endsec,istat,message)
  !**************************************************************************
  !*
  ...
  integer(ip)       :: nc,istat,timesec,endsec
  !
  logical               :: go_on
  character(len=s_mess) :: mymessage
  integer(ip)           :: ilen,itime1,itime2,ns,is,ic
  real   (rp)           :: rvoid
  integer(ip)           :: ivoid
  !
  !***  Initializations
  ...
  !
100 istat = -1
  mymessage = 'get_source_nclass: error opening the source file '//TRIM(fname)
  message(1:MIN(ilen,s_mess)) = mymessage(1:MIN(ilen,s_mess))
  return
  !
101 istat = -1
  mymessage = 'get_source_nclass: error reading the source file at time'
  ...
  return
  !
end subroutine get_source_nclass
